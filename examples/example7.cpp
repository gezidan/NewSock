/*
 * <Copyright Year Your name, optional@email>
 *
 * Copyright 2010 Adam Chyła, adam.chyla@gmail.com
 * All rights reserved. Distributed under the terms of the GNU Lesser General Public License v3.0.
 */

/*
 * SOCK_STREAM listener and talker example
 *
 * Usage:
 * example3 port
 */

#include "../include/Error.h"
#include "../include/Socket.h"

// C++ API headers
#include <cstring>
#include <iostream>
#include <sstream>

// POSIX API headers
#include <arpa/inet.h>
#include <sys/types.h>


using NewSock::Sock;
using NewSock::Error;


int main(int argc, char **argv)
{
    int port = 8010;

    if (argc > 1)
    {
        std::stringstream stream(argv[1]);
        stream >> port;

        if (port <= 0)
            port = 8010;

        std::cerr << "Port: " << port << '\n';
    }

    Sock soket(PF_INET, SOCK_STREAM, 0, NewSock::SockNoThrow);

    struct sockaddr_in myAddr;
    memset(&myAddr, 0, sizeof(myAddr));
    myAddr.sin_addr.s_addr = htonl(INADDR_ANY);
    myAddr.sin_port = htons(port);
    myAddr.sin_family = PF_INET;

    soket.Bind(reinterpret_cast<struct sockaddr*>(&myAddr));
    soket.Listen(20);


    struct sockaddr_in theirAddr;
    memset(&theirAddr, 0, sizeof(theirAddr));

    while (true)
    {
        Sock soket2(soket.Accept(reinterpret_cast<struct sockaddr*>(&theirAddr), NewSock::SockNoThrow));

        while (soket2.Recv(0, NewSock::SockNoThrow) != -1)
        {
            std::cout << soket2.GetBufLength() << ": ";

            for (int i = 0; i < soket2.GetBufLength(); i++)
                std::cout << soket2.GetBuf()[i];

            std::string line = "=> ";
            for (int i = 0; i < soket2.GetBufLength(); i++)
                line += soket2.GetBuf()[i];

            if (soket2.Send(line.c_str(), line.length(), NewSock::SockNoThrow) == -1)
                return 0;

            std::cout << '\n';
        }
    }

    return 0;
}
